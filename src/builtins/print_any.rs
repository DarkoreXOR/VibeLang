...
pub struct PrintAnyBuiltin;

impl PrintAnyBuiltin {
    fn fmt_value(v: &Value) -> String {
        match v {
            ...
            Value::Tuple(parts) => {
                let inner: Vec<String> = parts.iter().map(|x| Self::fmt_value(x)).collect();
                format!("({})", inner.join(", "))
            }
            Value::Array(parts) => {
                let inner: Vec<String> = parts.iter().map(|x| Self::fmt_value(x)).collect();
...
impl BuiltinImpl for PrintAnyBuiltin {
    ...
    fn validate_decl(
        ...
        if params.len() != 1 || type_name_named(&params[0].ty) != Some("Any") {
            return Err(BuiltinError::new(
                "internal `print_any` must be `print_any(a: Any);`",
                Some(name_span),
            ));
        }
        if return_type.is_some() {
            return Err(BuiltinError::new(
                "internal `print_any` must have no return type",
                Some(name_span),
            ));
        }
        Ok(())
    ...
    fn eval(&self, args: Vec<Value>, span: Span) -> Result<Value, BuiltinError> {
        if args.len() != 1 {
            return Err(BuiltinError::new("print_any needs 1 argument", Some(span)));
        }
